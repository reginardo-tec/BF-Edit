...
import { fetchSiteSettings, setMaintenanceMode, setUploadsMaintenance, type SiteSettings } from "@/lib/site-settings";
        ...
        <div className="overflow-hidden rounded-2xl border border-border/60 bg-background">
          <Tabs defaultValue="dashboard" className="w-full">
            <TabsList className="m-2">
              ...
              <TabsTrigger value="pedidos"><ShoppingBag className="mr-1.5 h-4 w-4" />Pedidos</TabsTrigger>
          ...
          {loading ? (
            ...
            <TabsContent value="pedidos" className="m-0 border-t border-border/60">
              <AdminOrdersPanel />
            </TabsContent>
          </Tabs>
        </div>
...
function MaintenancePanel() {
  ...
  const toggleUploads = async (enabled: boolean) => {
    ...
    try {
      await setUploadsMaintenance(settings.id, enabled);
      setSettings({ ...settings, uploads_maintenance: enabled });
      toast.success(enabled ? "Página de uploads em manutenção" : "Página de uploads liberada");
    } catch (e) {
      toast.error((e as Error).message);
    } finally {
  ...
  const uploadsEnabled = !!settings?.uploads_maintenance;
      ...
      <div className="rounded-2xl border border-border/60 bg-background p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <Wrench className="h-4 w-4 text-muted-foreground" />
              <h3 className="font-display text-xl">Página de envio de arquivos</h3>
              {uploadsEnabled && (
                <span className="rounded-full bg-[var(--coral)]/20 px-2 py-0.5 text-xs text-[var(--coral)]">
                  Em manutenção
                </span>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Controla apenas a página <strong>/enviar</strong>. Quando ativo, quem tentar enviar arquivos será redirecionado para a página de manutenção — independente do modo manutenção geral.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Switch checked={uploadsEnabled} disabled={savingUploads} onCheckedChange={toggleUploads} />
            <Label className="text-xs">{uploadsEnabled ? "Ativo" : "Inativo"}</Label>
          </div>
        </div>
      </div>
    </div>
